...
export default function InterestSelectModal({
  ...
  const [draftInterests, setDraftInterests] =
    useState<InterestKeyword[]>(interests);

  /**
   * 관심 분야 토글 함수
   * @param interest - 관심 분야
   */
  const toggleInterests = (interest: InterestKeyword) => {
    const exists = draftInterests.some(({ id }) => id === interest.id);

    if (exists) {
      setDraftInterests(draftInterests.filter(({ id }) => id !== interest.id));
      return;
    }

    if (draftInterests.length >= 5) {
      return;
    }
